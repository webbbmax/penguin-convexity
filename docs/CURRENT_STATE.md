...
- X01 的精确根因是 `app/c2-4-front.js` 的 `renderDetail` 将 `assetId` 与旧 `id` 放在同一查找表达式：URL 缺少旧 `id` 时 `params.get("id")` 为 `null`，首条 OpenServ 的 `projectId` 也为 `null`，因此在比较目标 `assetId` 前提前命中 OpenServ。正式普通用户路径的 Wrapped Gonka、cap、OpenServ 真实点击、刷新与返回上下文均已通过，70/70 身份合同和 6/6 定点合同通过。
