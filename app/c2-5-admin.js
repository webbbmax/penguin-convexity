(function c25ManagerControlPlane() {
  ...
  function ruleCounts(value) {
    ...
    return `输入 ${fmtValue(value.input, "0")} · 基线通过 ${fmtValue(value.baselinePassed, "0")} · 当前通过 ${fmtValue(value.effectivePassed, "0")} · 变化 ${fmtValue(value.changed, "0")}`;
  }
  ...
  function controlMarkup(task) {
    const controls = (task.controls || []).map((item) => {
      ...
      return `${parameter}<button class="c25-button" type="button" data-c25-control data-task-id="${esc(task.taskId)}" data-action="${esc(item.action)}" ${unavailable ? "disabled title=\"当前没有可重试对象\"" : ""}>${esc(actionLabels[item.action] || item.label || item.action)}</button>`;
    }).join("");
  ...
  function installControlHandlers(root = document) {
    $$('[data-c25-control]', root).forEach((button) => button.addEventListener("click", async () => {
      ...
      try {
        ...
        const dialog = openDialog(actionLabels[button.dataset.action] || "确认管理操作", `<p>系统尚未执行。请核对对象、前值、后值、影响和恢复方式。</p><dl class="c25-facts"><div><dt>任务</dt><dd class="c25-machine">${esc(item.taskId)}</dd></div><div><dt>当前状态</dt><dd>${esc(fmtValue(impact.before))}</dd></div><div><dt>拟改变值</dt><dd>${esc(fmtValue(impact.afterRequested))}</dd></div><div><dt>读写对象</dt><dd>${esc(fmtValue(impact.readWriteObjects))}</dd></div><div><dt>冲突任务</dt><dd>${esc(fmtValue(impact.conflictTaskIds, "无"))}</dd></div><div><dt>前台影响</dt><dd>${esc(fmtValue(impact.frontendImpact, "无"))}</dd></div><div><dt>恢复方式</dt><dd>${esc(fmtValue(impact.recovery))}</dd></div><div><dt>确认有效期</dt><dd>${esc(fmtTime(preview.expiresAt))}</dd></div></dl>`, actionLabels[button.dataset.action] || "确认执行", async () => {
  ...
  async function renderRules() {
    ...
    setMain(`${pageHeader("规则透明中心", "冻结基线、当前有效规则、活动覆盖和历史版本分开；C2.5不直接编辑规则。", null, data.observedAt)}<section class="c25-summary-grid"><article class="c25-card"><span>冻结基线</span><strong class="c25-machine">${esc(data.frozenBaseline?.ruleVersion)}</strong><small>哈希${data.frozenBaseline?.hashMatchesFrozen ? "匹配" : "不匹配"}</small></article><article class="c25-card"><span>当前有效规则</span><strong class="c25-machine">${esc(data.effective?.ruleVersion)}</strong><small>与代码值${data.effective?.reconciledWithCode ? "一致" : "待对账"}</small></article><article class="c25-card"><span>活动覆盖</span><strong>${data.activeOverride?.active ? "正在生效" : "未生效"}</strong><small>${esc(fmtTime(data.activeOverride?.approvedAt))}</small></article><article class="c25-card"><span>同输入变化</span><strong>${(replay.addedAssetIds || []).length + (replay.removedAssetIds || []).length}</strong><small>按assetId独立复算</small></article></section><section class="c25-chart" aria-labelledby="ruleImpactTitle"><div class="c25-chart-head"><div><h3 id="ruleImpactTitle">基线与有效规则通过数量</h3><p>口径：同一份完整跟踪输入，分别重放冻结基线和当前有效规则。</p></div><div><small>数据时间：${esc(fmtTime(data.observedAt))}</small><br><a href="#ruleExactTable">查看精确表格</a></div></div><div class="c25-bar"><span>冻结基线</span><i style="--c25-bar:${(replay.baselinePassedCount || 0) * 100 / max}%"></i><strong>${replay.baselinePassedCount || 0}</strong></div><div class="c25-bar"><span>当前有效</span><i style="--c25-bar:${(replay.effectivePassedCount || 0) * 100 / max}%"></i><strong>${replay.effectivePassedCount || 0}</strong></div><small>筛选：当前完整输入 · 空值不补零 · 综合分不控制资格或凸性线索。</small></section><section class="c25-panel"><div class="c25-panel-header"><div><h2>规则列表</h2><p>15%、20%、10%与六项停用门槛保留原始证据。</p></div><a href="decision-trace.html">按assetId解释</a></div><div class="c25-table-wrap" id="ruleExactTable"><table class="c25-table"><thead><tr><th>规则</th><th>冻结基线</th><th>当前有效</th><th>覆盖状态</th><th>影响数量</th><th>真实样本</th></tr></thead><tbody>${(data.rules || []).map((row) => `<tr><td><strong>${esc(row.plainName)}</strong><small class="c25-machine">${esc(row.ruleId)}</small><small>${esc(row.plainDescription)}</small></td><td>${esc(ruleValue(row.baselineValue, row.unit))}</td><td>${esc(ruleValue(row.effectiveValue, row.unit))}</td><td>${esc(row.difference)}<small>${esc(approvalLabel(row.approvedBy))} · ${esc(fmtTime(row.approvedAt))}</small></td><td>${esc(ruleCounts(row.counts))}</td><td>通过：${esc(ruleSample(row.passExample))}<small>未通过：${esc(ruleSample(row.nonPassExample))}</small></td></tr>`).join("")}</tbody></table></div></section><section class="c25-panel"><div class="c25-panel-header"><div><h2>版本历史与边界</h2><p>${esc(data.bayesBoundary)}</p></div></div><div class="c25-table-wrap"><table class="c25-table"><thead><tr><th>版本</th><th>状态</th><th>来源哈希</th></tr></thead><tbody>${(data.history || []).map((row) => `<tr><td class="c25-machine">${esc(row.version)}</td><td>${row.status === "frozen_baseline" ? "冻结基线" : row.status === "active" ? "当前生效" : esc(row.status)}</td><td class="c25-machine">${esc(fmtValue(row.sourceSha256))}</td></tr>`).join("")}</tbody></table></div></section>`);
  }
